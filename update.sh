...
HSM_PREVIOUS_VERSION=$(cat "$INSTALL_DIR/VERSION" 2>/dev/null || printf unknown)
    ...
    for setting in HSM_STORAGE_HIDE_USB_FLASH HSM_STORAGE_EXCLUDE_SERIALS HSM_STORAGE_EXCLUDE_MODELS HSM_RAID_SSACLI_ENABLED HSM_SSACLI_HELPER HSM_SSACLI_USE_SUDO HSM_PROXMOX_ENABLED HSM_PROXMOX_REQUIRED HSM_PVEVERSION_BINARY HSM_PROXMOX_CPU_SAMPLE_SECONDS HSM_COOLING_ENABLED HSM_COOLING_REQUIRED HSM_COOLING_X8FAN_HELPER HSM_COOLING_X8FAN_USE_SUDO HSM_COOLING_INFLUX_URL HSM_COOLING_INFLUX_DATABASE HSM_COOLING_DISK_MAX_AGE_SECONDS HSM_COOLING_CONTROL_STATE_FILE HSM_COOLING_STATUS_INTERVAL_SECONDS; do
        if ! grep -q "^${setting}=" "$DEFAULTS_FILE"; then
            case "$setting" in
                ...
                HSM_COOLING_STATUS_INTERVAL_SECONDS) value=600 ;;
...
render_dashboards() {
    ...
    python3 - "$INSTALL_DIR/dashboard" "$GRAFANA_DASHBOARD_DIR" "$datasource_uid" <<'PY'
import json
import os
import sys
...
PY
    ...
    find "$GRAFANA_DASHBOARD_DIR" -type d -exec chmod 0755 {} +
    find "$GRAFANA_DASHBOARD_DIR" -type f -name '*.json' -exec chmod 0644 {} +
}
...
[ "$(id -u)" -eq 0 ] || fail "This updater must be run as root."
...
DATASOURCE_UID=$(detect_influxdb_uid) || fail "No InfluxDB datasource was found. Set HSM_GRAFANA_DATASOURCE_UID in $DEFAULTS_FILE."
...
printf '%s\n' "Update complete."
printf '%s\n' "Existing cache and configuration were preserved."
printf '%s\n' "Grafana dashboards were updated automatically."
